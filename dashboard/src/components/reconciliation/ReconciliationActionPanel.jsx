...
export default function ReconciliationActionPanel({
  ...
  return (
    <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-4">
      <div>
        <span className="badge badge-purple mb-1">Portal Observation Review</span>
        <h4 className="font-bold text-slate-900 text-sm">Explain and quarantine the discrepancy</h4>
        <p className="text-xs text-slate-500 mt-0.5">
          Record exact portal behavior for evidence-backed review. This action does not change deterministic rules or NotebookLM sources.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-200">
        ...
        >
          ...
          {isSubmittingDelta ? 'Recording...' : 'Quarantine Observation for Review'}
        </button>
      </div>
      ...
      {deltaSyncSuccess && (
        <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-xl text-xs text-emerald-800 flex items-center gap-2">
          ...
          <span>Portal observation recorded in product-scoped quarantine. Active knowledge remains unchanged.</span>
        </div>
      )}
    </div>
  );
