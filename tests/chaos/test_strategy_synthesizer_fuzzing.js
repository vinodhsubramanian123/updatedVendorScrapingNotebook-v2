...
test('Chaos & Adversarial Fuzzing: Strategy Synthesizer & Workload DNA', async (t) => {
  ...
  await t.test('extractWorkloadDna - Extreme and Adversarial BOQs', () => {
    ...
    // We expect some basic recovery or at least not crashing.
    // The processor has 32 cores * 2 = 64 cores.
    // Memory is 64 * 4 = 256GB.
    // GPU is present but qty is NaN (which in code resolves to NaN or 1 if it falls back to 1... actually code says `it.quantity || 1`)
  ...
  await t.test('synthesize5TierRankedSolutions - 0-SKU empty list', () => {
    ...
    res.forEach((rank, i) => {
      ...
      assert.ok(rank.ragSecondOpinion.startsWith('✅ Grounded in QuickSpecs'));
    });
  });
  ...
  await t.test('synthesize5TierRankedSolutions - Missing/Corrupted strategy_addons.json', (t2) => {
    // We will stub fs.readFileSync to throw an error for strategy_addons.json
    // to simulate corruption/deletion.
    const originalReadFileSync = fs.readFileSync;
    const originalExistsSync = fs.existsSync;

    t2.mock.method(fs, 'existsSync', (p) => {
      if (p.includes('strategy_addons.json')) return true;
      return originalExistsSync(p);
    });

    t2.mock.method(fs, 'readFileSync', (p, enc) => {
      if (p.includes('strategy_addons.json')) {
  ...
  await t.test('synthesize5TierRankedSolutions - Duplicate ranks prevention', () => {
    // If the strategy synthesizer accidentally collapses tiers, we want to catch it.
    // The synthesizer has a deduplication filter.
    // We should ensure it returns exactly 5 distinct ranks.
    ...
    // Make sure tier names are distinct conceptually
