...
- **Autonomous NotebookLM / RAG Execution (Auto-Approved):** All queries and interactions with Google NotebookLM (via `gemini-notebook-mcp` or `nlm` CLI) are pre-authorized and auto-approved across all scraping, verification, knowledge sync, and BOQ evaluation workflows. Agents MUST NOT prompt the human user for permission to execute NotebookLM queries; execute them autonomously.
