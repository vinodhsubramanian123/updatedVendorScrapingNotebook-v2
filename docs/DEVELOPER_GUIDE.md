...
# Install Playwright Chromium binary (Required for E2E headless tests)
npx playwright install chromium

# Install CLI tools via uv (or pip install notebooklm-mcp-cli graphifyy)
uv tool install notebooklm-mcp-cli
uv tool install graphifyy
```

### Cross-Machine Autonomous Migration (`npm run restore:env`)
If you are migrating this repository to a new machine (MacBook Air with macOS 12.7 Monterey, Windows 10/11, or Linux) and have the `antigravity_migration_bundle.zip` generated by Antigravity AI, **do not configure anything manually**:
...
3. The restorer automatically:
   - Detects the OS and user home directory.
   - Extracts the bundle cross-platform.
   - Restores `.env` with Gemini key pools and rotation state.
   ...
   - Generates machine-accurate `~/.gemini/config/mcp_config.json` tailored to the local host paths.
...
> **Chrome & CDP Port 9222**: The scraping engine (`browser_launcher.js`) automatically locates Chrome across standard macOS (`/Applications/Google Chrome.app`) and Windows (`%PROGRAMFILES%\Google\Chrome` or `%LOCALAPPDATA%\Google\Chrome`) locations and launches with `--remote-debugging-port=9222`.
