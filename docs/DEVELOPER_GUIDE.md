...
# Install Playwright browser binaries (Required for E2E headless tests)
npx playwright install

# Install Graphify globally (Required for codebase semantic mapping)
npm install -g graphify
```

### Cross-Machine Agentic Migration & Restoration
If you are migrating this repository to a new machine and have the `antigravity_migration_bundle.zip` generated by Antigravity AI, **do not set this up manually**.
1. Extract the zip bundle.
2. The AI Agent will autonomously read the `ANTIGRAVITY_RESTORE_INSTRUCTIONS.md` from the bundle.
3. It will autonomously move `.env`, Google ADC (`gcloud`), MCP config (`gemini_config`), and live brain state (`history`) to their exact OS-agnostic paths.

> **Important Chrome Setup Note**: The CDP scraping engine (`browser_launcher.js`) strictly targets default OS installation paths for Google Chrome (e.g., `/Applications/Google Chrome.app/Contents/MacOS/Google Chrome` or `C:\Program Files\Google\Chrome\Application\chrome.exe`). Do not install Chrome in a custom directory.
