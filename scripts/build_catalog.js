...
const { HPE_SKU_EXTRACT_REGEX, cleanBaseSKU, classifyOptionType, isServiceSku } = require('../lib/catalog/sku.js');
...
for (let ti = 0; ti < expandedTables.length; ti++) {
  ...
  if (!subCat || subCat === '(Sub-table)') {
    const { synthesizeSubcategoryName } = require('../lib/catalog/product_meta.js');
