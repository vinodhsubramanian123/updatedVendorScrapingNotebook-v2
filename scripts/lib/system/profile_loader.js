...
function loadProfileSync(family, gen) {
  const cacheKey = `${family}_${gen}`.toLowerCase();
  if (_profileCacheSync.has(cacheKey)) {
    return _profileCacheSync.get(cacheKey);
  }
  ...
  _profileCacheSync.set(cacheKey, mergedProfile);
