...
async function syncRunningKnowledge(options = {}) {
  ...
  // 2. Collect existing deltas and imported markdown rules
  logger.info('RUNNING_KNOWLEDGE', 'Collecting local deltas and scanning imported source guides...');
  const existingDeltas = collectAllDeltas(OUTPUTS_DIR);
  const importedRules = scanImportedSources();
  logger.info('RUNNING_KNOWLEDGE', `Collected ${existingDeltas.length} catalog deltas and ${importedRules.length} parsed source rules.`);

  // 3. Deduplicate rules non-destructively (INV-13)
  const deduplicatedRules = deduplicateRules(existingDeltas, importedRules);
  logger.info('RUNNING_KNOWLEDGE', `Deduplicated rules count: ${deduplicatedRules.length} (0 deleted, all history preserved).`);

  // 4. Update master_knowledge_registry.json locally
  logger.info('RUNNING_KNOWLEDGE', 'Updating master_knowledge_registry.json...');
  const universalRules = deduplicatedRules.filter(r => r.scopeTaxonomy === 'UNIVERSAL_VENDOR');
  const familyGenRules = deduplicatedRules.filter(r => r.scopeTaxonomy === 'FAMILY_GEN');
  const chassisSpecificRules = deduplicatedRules.filter(r => r.scopeTaxonomy !== 'UNIVERSAL_VENDOR' && r.scopeTaxonomy !== 'FAMILY_GEN');
  ...
  // 5. Generate Running Knowledge Charter Markdown
  ...
  fs.writeFileSync(UNIVERSAL_CHARTER_PATH, charterMarkdown, 'utf8');
  logger.info('RUNNING_KNOWLEDGE', `Wrote local charter markdown to ${RUNNING_CHARTER_PATH} (${charterMarkdown.length} bytes).`);
